...
public class ZipUtils
{
	...
	/**
	 * Archive a Source path into a zip file, created at the same level as the source path
	 * @param source
	 * @param target
	 */
	...
	public static void zip(String source, String target) throws TestingException
	{
		...
		else
		{
			...
			try
			{
				...
				// Handle case where target is not specified - create file at the same level as source
				...
				{
					targetFile = sourceDirObj.getParent() + File.separator
							+ sourceDirObj.getName() + ".zip";
				}
	...
	public static void unZip(String zipFilePath, String targetFolderPath) throws TestingException
	{
		...
		else
		{
			...
			try
			{
				...
				String fullTargetPath = (StringUtils.isNotBlank(targetFolderPath) ?
											targetFolderPath : new File(zipFilePath).getParent())
										+ File.separator
				...
				CucumberLogUtils.logDebug("File [" + zipFilePath + "] has been unzipped to folder ["
						+ fullTargetPath + "]");
			}
			catch (IOException e)
			{
	...
	/**
     * Uncompresses the GZip compressed bytes to String.
     * Returns null if input was null
...
//				ByteArrayInputStream byteStream = new ByteArrayInputStream(inputBytes);
	...
	/**
     * Uncompresses the GZip compressed bytes to byte array.
     * Returns null if input was null
	 * @param inputBytes GZip compressed bytes
	 * @return UnCompressed Bytes
	 * @throws TestingException
	...
	public static byte[] gZipDecompressBytesToBytes(byte[] inputBytes) throws TestingException
	{
		...
		if (inputBytes != null)
		{
			...
			{
				throw new TestingException(TestingErrorCode.CONTENT_UNCOMPRESS_ERROR,
						e.getMessage());
			}
